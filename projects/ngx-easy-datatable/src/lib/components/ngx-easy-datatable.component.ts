...
@Component({
  selector: 'ngx-easy-datatable',
  templateUrl: 'ngx-easy-datatable.component.html',
  styleUrls: ['ngx-easy-datatable.component.scss']
})
export class NgxEasyDatatableComponent implements OnInit {
  @Input() public viewModel: any;
  @Input() public data: Observable<any>;
  @Input() public config: any;
  @Input() public headers: any = [];
  @Input() public actions: any = [];
  @Output() public onActionClick = new EventEmitter<any>();
  ...
  public ngOnInit() {
    ...
    if (this.data) {
      this.data.subscribe((data) => {
        ...
        this.allData = this.tableData = data;
        this.setPageNumbers();
      });
    }
  }
      ...
      if (!column) {
        ...
        this.tableData = this.allData.filter((d) => {
          ...
          if (column.type === 'date' || column.type === 'datetime') {
            return comparator
              ? new Date(comparator).toDateString().indexOf(new Date(search).toDateString()) > -1
              : null;
          } else {
  ...
  public onEvent(data, index, action) {
    this.onActionClick.emit({ data, index, action: action.event_name });
  }
